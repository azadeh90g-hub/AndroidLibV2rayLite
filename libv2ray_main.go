...
func measureRequestDelay(ctx context.Context, client *http.Client, targetURL string) (int64, error) {
	...
	// Validate URL scheme to prevent SSRF or other protocol attacks
	parsedUrl, err := url.Parse(targetURL)
	if err != nil {
		return -1, fmt.Errorf("invalid URL: %w", err)
	}
	if parsedUrl.Scheme != "http" && parsedUrl.Scheme != "https" {
		return -1, fmt.Errorf("unsupported protocol scheme: %s", parsedUrl.Scheme)
	}
	...
	for i := 0; i < attempts; i++ {
		...
		// Process response in a closure to ensure the body is closed in each iteration
		// and to limit the response body read size to prevent DoS.
		err = func() error {
			...
			// Limit reading to 1MB to prevent DoS attacks
